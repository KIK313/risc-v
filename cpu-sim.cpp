...
int main() {
  //  freopen("a.in","r",stdin);
    ...
    register_file rf; reorder_buffer rob; reservation_station rs;
    co.init(&memo); iu.init(Mem, &rob, &rs, &rf, &co, &pc); memo.init(Mem, &rs, &co);
    pc.init(&co); rob.init(&rs, &co, &rf, &memo, &pc); rs.init(&rob, &al, &memo, &co, &pc);
    rf.init(&co);
    int cl = 0;
  //  cerr<<cl<<" -100"<<endl;
    while (!co.ed()) {
        ...
        rf.work_clk();
       // if (cl>9999500)   cerr<<cl<<" CLOCK TIME -100"<<endl;
        co.neg_edge(); iu.neg_edge(); memo.neg_edge(); pc.neg_edge();
        rf.neg_edge(); rob.neg_edge(); rs.neg_edge();
    }    
   // cerr<<cl<<" -100"<<endl;    
   cout<<(rf.getans()&255);       
